...
async function searchMercadoLivre(title,platform){
  const token=process.env.MELI_ACCESS_TOKEN;
  if(!token)return {status:'not_configured',offers:[]};
  try{
    const params=new URLSearchParams({q:sourceQuery(title,platform),limit:'20',condition:'used'});
...
module.exports=async function handler(req,res){
  ...
  if(condition!=='used')return res.status(400).json({error:'unsupported_condition'});

  const meli=await searchMercadoLivre(title,platform);
