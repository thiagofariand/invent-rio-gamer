...
function rowMarkup(r){
  ...
  return `<article class="row" data-rowkey="${rk}">
    <a class="row-cover" href="${href}" tabindex="-1" aria-hidden="true" data-covercell>${coverTile(p.title,{platform:'',image:bestOfferImage(p,r.platform)})}</a>
...
async function autoloadPrices(rows,token){
  ...
  await mapLimit(need,2,async r=>{
    ...
    const cover=row&&row.querySelector('[data-covercell]');
    if(cover){const img=bestOfferImage(r.p,r.platform);if(img)cover.innerHTML=coverTile(r.p.title,{image:img});}
  });
}
