...
async function buildOffersResponse({method,query,token}){
  ...
  if(condition==='used'){
    // sites/MLB/search (usado por searchUsedListings) está retornando 403 pra
    // aplicações não certificadas — confirmado em 24/09/2026, não é problema
    // de token (outros endpoints com o mesmo token funcionam normalmente).
    // Vários relatos independentes de outros desenvolvedores mostram o mesmo
    // padrão desde março/2026, com o próprio suporte do Mercado Livre dizendo
    // que dúvidas de API dependem do "nível de certificação" da aplicação.
    // Em vez de gastar uma chamada que sempre falha, assume-se a limitação e
    // devolve um link de busca direto — mesmo tratamento já usado pra
    // Nintendo Store/PlayStation Store quando não há API de preço ao vivo.
    // Se o Mercado Livre liberar o endpoint de novo, é só voltar a chamar
    // searchUsedListings aqui (a função continua abaixo, intacta).
    return {statusCode:200,cacheControl:'s-maxage=3600, stale-while-revalidate=86400',body:{
      title,platform:platform||null,condition,
      offers:[],nationalOffers:[],importedOffers:[],totalShown:0,
      ...
      sources:{
        mercado_livre:{
          ...
          detail:'Busca de usados do Mercado Livre reservada a aplicações certificadas. Use o link de busca direta.'
        }
      }
    }};
  }
