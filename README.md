...
`buildOffersResponse` não chama mais `searchUsedListings` pra `condition=used` — devolve direto um link de busca externo (`externalSearchUrl`), mesmo tratamento já usado pra Nintendo Store/PlayStation Store. A função `searchUsedListings` continua no arquivo, intacta, pro caso do endpoint ser liberado de novo — só não é mais chamada automaticamente.

**Novo (2023/2020/2019)** continua funcionando normalmente — usa a API de Catálogo, que não tem essa restrição.
