# Inventário Gamer 2.0.1 — base consolidada

Base consolidada do Inventário Gamer: preserva o layout atual/legado aprovado e reúne as integrações e regras de dados definidas para a 2.0, sem depender de patches soltos.
...
- **Layout atual preservado:** header compacto com logo + busca + navegação, Home em painéis `Em alta` + `Games e ofertas` + `Merch & fan-made`, conteúdo central de ~1200 px e densidade visual das versões 0.98/0.99.
- Home, diretório Games, Universo, página individual do jogo, Em alta, áreas de merch/fan-made e Meu Inventário.
...
- Mercado Livre isolado atrás de `/api/offers`, com busca de produto novo via catálogo/buy-box quando disponível, fallback tolerante e busca de usados tratada como atalho externo quando não há dado confiável.
...
## Regra visual da reconstrução

**Aparência e densidade = versões antigas. Arquitetura e dados = 2.0.**

A Home preserva a composição atual `Em alta` à esquerda e `Games e ofertas` + `Merch & fan-made` à direita, sem introduzir um hero institucional novo. O Universo usa hero compacto + abas + grade de jogos imediatamente visível. A página individual do jogo volta à composição do mock antigo: hero horizontal baixo, gradiente escuro prolongado na área do texto, título **Buscar nas lojas**, bloco unificado `Físico + Digital` com a capa à esquerda e módulos menores de `Colecionáveis e merch` e `Fan-made` ao lado.

O logo histórico foi restaurado em `assets/brand/inventario-logo-header.png`.
...
- A integração ML não faz refresh persistente automático ainda; token expirado degrada para atalhos sem inventar preço.
