...
function renderHome(){
  ...
  <section class="home-feature" aria-labelledby="home-feature-title">
    <div class="home-feature-art game-hero-art" style="--hero-h:${hashStr(lead.title)%360}" aria-hidden="true"><div class="game-hero-placeholder"><b>${esc(initialsOf(lead.title))}</b><span>DESTAQUE EM ALTA</span></div></div>
...
function universeMedal(p){
  ...
  return `<a class="universe-medal ${status}" href="#/jogo/${p.slug}">
    <span class="medal-art" style="--h:${hashStr(p.title)%360}" data-igdb-medal data-igdb-title="${esc(p.title)}" data-igdb-platform="${esc(p.variants?.[0]?.[1]||'')}" data-igdb-year="${esc(p.year||'')}"><b>${esc(initialsOf(p.title))}</b>${badge?`<i>${esc(badge)}</i>`:''}</span>
