...
function renderHome(){
  ...
  const slides=trend.slice(0,3),top=trend.slice(1,4);
...
function universeOfferCard(p){
  ...
  return `<article class="universe-offer-card">
    <a class="universe-offer-cover igdb-cover-slot" href="#/jogo/${p.slug}?plat=${enc(platform)}" data-igdb-cover data-igdb-title="${esc(p.title)}" data-igdb-platform="${esc(platform)}" data-igdb-year="${esc(p.year||'')}">${coverTile(p.title,{note:false})}</a>
...
function renderUniverse(slug,params,token){
  ...
  if(tab==='games'){
    ...
    body=titles.length?`<section class="universe-inventory universe-inventory-compact" aria-labelledby="universe-games-title"><div class="universe-inventory-copy"><span class="eyebrow">SUA COLEÇÃO</span><h2 id="universe-games-title">Seu inventário de ${esc(u.name)}</h2><p>${owned} de ${titles.length} jogos marcados como Tenho${want?` · ${want} na lista Quero`:''}.</p></div><div class="universe-medals">${titles.slice(0,4).map(universeMedal).join('')}</div><a class="universe-inventory-all" href="#/universo/${slug}?tab=games">Ver todos →</a></section>
